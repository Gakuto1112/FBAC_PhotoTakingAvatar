...
- 撮影時に便利な分割線（縦横それぞれ3分割）が表示されます。
  - F1キーでUIを消すと一緒に隠れます。
  - 分割線を増減させたい場合は、`src/scripts/screen_split_line.lua`内の[`X_COUNT`](./src/scripts/screen_split_line.lua#L5)および[`Y_COUNT`](./src/scripts/screen_split_line.lua#L6)を変更してください。
...
2. [`src/models/main.bbmodel`](./src/models/main.bbmodel)を[BlockBench](https://www.blockbench.net)で開きます。
   ...
   - インポートしたFBACアバターのテクスチャファイルはFBAC_PhotoTakingAvatar側で別途保存しておくことをお勧めします。
     ...
     なお、Figuraでは同名の名前のテクスチャファイルの共存は許容していないため、`${オリジナル名}_${キャラクター名}.png`など、複数人キャラクターをインポートしてもテクスチャファイル名が重複しないように名付けます。
...
5. `src/scripts/avatar.lua`をメモ帳などで開き、[`BASE_POS`](./src/scripts/avatar.lua#L3)と書かれた行を探してください（始めのほうにあります）。
   ここを手順4で控えたワールド座標に置き換えます。
   `vectors.vec3(${X座標}, ${Y座標}, ${Z座標})`と置き換えてください。
   - ターゲットしたブロックの上面中心にキャラクターを立たせたい場合、ブロック座標に対してそれぞれ`(x, y, z) = (0.5, 1.0, 0.5)`を加えてください。

6. スクリプトの追加処理が必要であれば`src/scripts/avatar.lua`内の[`onInit()`](./src/scripts/avatar.lua#L25-L30)内に任意のコードを追加してください。
   この関数は、アバターの読み込み完了後に1度だけ実行されます。
   - もちろん、この関数の外にコードを書いてもらっても構いません。
     ただし、自分が何をやっているのか理解していない場合は、関数内に書くことをお勧めします。

7. あとは自分自身が動いていい感じの場所を探して、スクリーンショットを撮影してください。
...
### 「Model "main" contains textures with duplicate name ...」というエラーが表示され、モデルファイルが読み込まれない

インポートしたFBACアバターのテクスチャの名前が重複しています。
ファイル名が重複しないように、それぞれのテクスチャファイルに名前を付けて保存してください。

詳しくは「使い方」の章の[手順2](#使い方)を確認してください。

### 周りは明るいのにキャラクターだけ黒くなる

キャラクターの足元付近がフルブロックで埋まるとそのようになります。

`src/scripts/avatar.lua`内の[`CHARACTER_LIGHTING_BASE_POINT_OFFSET`](./src/scripts/avatar.lua#L8)において、Y座標を上げる（〜1程度）と改善されます。

### ヘイローが上下にアニメーションしない

静止画の撮影を想定しているため、ヘイローが上下に浮遊するアニメーションは省略しています。

### 固有武器のモデルが小さすぎる/大きすぎる

FBACキャラクターの固有武器のモデルの全長は全て同じ大きさに揃えられており（BlockBench上で16マス/マインクラフト上で1ブロック）、ゲーム内でそれぞれの大きさに適合するようにスケーリングされます。
そのため、そのままモデルをインポートすると大きさに違和感が生じてしまいます。

BlockBench内で固有武器のモデルのルートパーツ（`Gun`）を選択し、「Transform」→「スケール...」を選択すると任意の大きさにスケーリングできます。
スライダーを操作して、適切な大きさになるように調整してください。

なお、FBAC内で設定されている固有武器のスケールは、FBACレポジトリの`src/avatars/${キャラクター名}/scripts/blue_archive_character.lua`内にある、`BlueArchiveCharacter.gun.scale`で確認できます。

### 影MOD使用時に自分自身の影が邪魔になる

`src/scripts/avatar.lua`内の[`onInit()`](./src/scripts/avatar.lua#L25-L30)内に以下のコードを追加してください。
